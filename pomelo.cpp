...
template <typename T>
void pomelo::fund_project(const T& table, const name project_id, const name from, const name to, const extended_asset ext_quantity, const string memo )
{
    const auto project = table.get(project_id.value, "pomelo::fund_project: project not found");

    check(project.status == "ok"_n, "pomelo::fund_project: project not available for funding");
    check(project.accepted_tokens.count(ext_quantity.get_extended_symbol()), "pomelo::fund_project: not accepted tokens for this project");
...
void pomelo::fund_grant(const name grant_id, const extended_asset ext_quantity, const name user_id, const double value )
{
    ...
    check(round_id > 0, "pomelo::fund_grant: no funding round ongoing");

    // get round
    pomelo::rounds_table rounds( get_self(), get_self().value );
    const auto round_itr = rounds.find( round_id );
    check( round_itr->grant_ids.count( grant_id ), "pomelo::fund_grant: grant is not part of this funding round");
